...
Page({
  ...
  radioChange(e) {
    ...
    wx.request({
      ...
      success(res) {
        ...
        var anes_analysis_dict = self.data.analysis_dict
        anes_answer_dict[e.detail.value.split("/")[2]] = res.data.answer
        anes_analysis_dict[e.detail.value.split("/")[2]] = res.data.analysis
        self.setData({
          answer_dict: anes_answer_dict,
          analysis_dict: anes_analysis_dict
        })
        //获取用户选的答案
  ...
  nextlastswiper(e) {
    ...
    if (self.data.already.indexOf(e.detail.current) === -1) {
      ...
      var analysis = self.data.analysis_dict[e.detail.current]
      self.setData({
        disabled: true,
        answer: "正确答案" + answer,
        analysis: "本题解析:" + analysis,
      })
    }
